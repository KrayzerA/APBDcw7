...
public interface IDbService
{
    ...
    Task<int?> AddWarehouseProduct(Product product, Warehouse warehouse, Order order);
    Task<int?> AddWarehouseProductAsync(int idProduct, int idWarehouse, int amount, DateTime date);
    Task<int?> AddProductToWarehouse(int idProduct, int idWarehouse, int amount, DateTime date);
}
...
public class DbService(IConfiguration configuration) : IDbService
{
    ...
    public async Task<Product?> GetProduct(int id)
    {
        ...
        var com = new SqlCommand("SELECT p.Description, p.Name, p.Price FROM Product p Where IdProduct = @1",
            connection);
    ...
    public async Task<Order?> GetOrder(int id)
    {
        ...
        var com = new SqlCommand(
            "Select o.IdProduct, o.Amount, o.CreatedAt, o.FulfilledAt FROM [Order] o where IdOrder = @1", connection);
        com.Parameters.AddWithValue("@1", id);
        var reader = await com.ExecuteReaderAsync();
        if (!reader.HasRows)
        {
    ...
    public async Task<Order?> GetOrderWithProduct(int idProduct, int amount, DateTime data)
    {
        ...
        var com = new SqlCommand("Select o.IdOrder, o.IdProduct, o.Amount, o.CreatedAt, o.FulfilledAt FROM [Order] o " +
                                 "Where o.IdProduct = @1 and o.Amount = @2 and CreatedAt < @3 and FulfilledAt IS NULL",
            connection);
    ...
    public async Task<Boolean> CheckProductWarehouse(int idOrder)
    {
        await using var connection = await GetConnection();

        var com = new SqlCommand("select pw.IdProductWarehouse from Product_Warehouse pw where IdOrder = @1",
            connection);
    ...
    public async Task<int?> AddWarehouseProduct(Product product, Warehouse warehouse, Order order)
    {
        ...
        try
        {
            ...
            com.Parameters.AddWithValue("@1", warehouse.IdWarehouse);
            com.Parameters.AddWithValue("@2", product.IdProduct);
            com.Parameters.AddWithValue("@3", order.IdOrder);
            com.Parameters.AddWithValue("@4", order.Amount);
            com.Parameters.AddWithValue("@5", order.Amount * product.Price);
    ...
    public async Task<int?> AddProductToWarehouse(int idProduct, int idWarehouse, int amount, DateTime date)
    {
        await using var connection = await GetConnection();
        var com = new SqlCommand("AddProductToWarehouse", connection);
        com.CommandType = CommandType.StoredProcedure;
        com.Parameters.AddWithValue("@IdProduct", idProduct);
        com.Parameters.AddWithValue("@IdWarehouse", idWarehouse);
        com.Parameters.AddWithValue("@Amount", amount);
        com.Parameters.AddWithValue("@CreatedAt", date);
        var id = (int) (await com.ExecuteScalarAsync())!;
        return id;
    }
}